...
//
// March 16, 2024 - Teepanis Chachiyo
//     Add flag to include nuclei contribution
//
void uhf_potential_XYPlane(
	...
	double dx, double dy,               // step size
	int nx, int ny,                     // number of points
	int inc_nuclei,                     // flag to include nuclei contribution
...
//
// March 16, 2024 - Teepanis Chachiyo
//         Add options to include nuclei contribution or not
//
...
// uhf_getValue: compute quantity such as electron density, or gradient at a point.
// Type of quantity is set according to outVolumeType from option.h  
//
// Mar 8, 2024 - Teepanis Chachiyo
//         Initial Implementation, copied from uhf_GradRhoOverRho43
//
// March 16, 2024 - Teepanis Chachiyo
//         Add potential options
//
double uhf_getValue(
const int type,                                  // type of value to return
const int nBasis,                                // number of basis function
const struct option_t *opt,                      // global options
const struct GTOBasis_t *gto,                    // function structure
const struct Molecule_t *mol,                    // molecule structure
const double *PA, const double *PB,              // density matrix
const double x, const double y, const double z){ // coordinate to get the value

	int i,j,c;
	double sum;
	double value;

	// the variable naming is the same as getXCDirect_LibXCcompat in dft.c
	double *VAL;                   // basis function at the point
	double *VALX,*VALY,*VALZ;      // gradient of basis function at the point
	...
	if(VAL==NULL || VALX==NULL || VALY==NULL || VALZ==NULL){
		printf("uhf_getValue - error cannot allocate memory\n");
		exit(-1);
	}
	...
	// enforce positive value to ensure numerical stability
	rhoa = fabs(rhoa);
	rhob = fabs(rhob);

	// compute possitive kinetic energy density
	double taua = 0.0, taub = 0.0;

	for(i=0; i < nBasis; i++)
	for(j=0; j < nBasis; j++){
		double gDOTg = VALX[i]*VALX[j] + VALY[i]*VALY[j] + VALZ[i]*VALZ[j];
		taua += PA[i*nBasis+j]*gDOTg/2.0;
		taub += PB[i*nBasis+j]*gDOTg/2.0;
	}
	...
	if(opt->volumeFunc1Id) xc_func_init(&func1, opt->volumeFunc1Id, XC_POLARIZED);
	if(opt->volumeFunc2Id) xc_func_init(&func2, opt->volumeFunc2Id, XC_POLARIZED);
#endif

	double Exc=0.0, dEdrhoa=0.0, dEdrhob=0.0, Ga=0.0, Gb=0.0;
...
// 1/2(3PI^2)^(1/3) >> python code 1/2/math.pow(3*math.pi**2,1/3)
#define S_COEF 0.1616204596739955

	case VOLUME_S_TOTAL:
		value = sqrt((rhoax+rhobx)*(rhoax+rhobx)+
				     (rhoay+rhoby)*(rhoay+rhoby)+
				     (rhoaz+rhobz)*(rhoaz+rhobz));
		value = value/pow(rhoa+rhob,4.0/3.0);
		value = value * S_COEF;
	break;
	...
	case VOLUME_GRAD_RS:
		value = sqrt((rhoax+rhobx)*(rhoax+rhobx)+
				     ...
				     (rhoaz+rhobz)*(rhoaz+rhobz));
		value = value/pow(rhoa+rhob,4.0/3.0);
// (36*PI)^(1/3) >> python code (36*math.pi)**(1/3)
#define RSFACTOR 4.835975862049408
		value = value/RSFACTOR;
	break;

 	case VOLUME_TAU_TOTAL:
		value = taua + taub;
	break;

	case VOLUME_POTENTIAL_TOTAL:
		value = uhf_potential(nBasis,gto,mol,PA,PB,0,x,y,z);
	break;

	case VOLUME_EX_CHACHIYO_TOTAL:
		Exc=0.0, dEdrhoa=0.0, dEdrhob=0.0, Ga=0.0, Gb=0.0;
		getDFT_xGGA_Chachiyo(rhoa, rhob,
					sqrt(rhoax*rhoax+rhoay*rhoay+rhoaz*rhoaz),
					sqrt(rhobx*rhobx+rhoby*rhoby+rhobz*rhobz),
					&Exc, &dEdrhoa, &dEdrhob, &Ga, &Gb);
		value = Exc;
	break;

#ifdef LIBXC
	case VOLUME_LIBXC_TOTAL:
		Exc=0.0;
	...
	case VOLUME_EX_HF_TOTAL:

		; // fix older gcc compiler syntax issue
		int p,q;

		// build potential matrix
		double *Vqj=calloc(nBasis*nBasis, sizeof(double));
		double *VqA=calloc(nBasis, sizeof(double));
		double *VqB=calloc(nBasis, sizeof(double));
		if(Vqj==NULL || VqA==NULL || VqB==NULL){
			printf("uhf_getValue - error cannot allocate memory\n");
			exit(-1);
		}

		// according how nai() is implemented in int.c, it returns negative values
		for(q=0; q < nBasis; q++)
		for(j=0; j < nBasis; j++)
			Vqj[q*nBasis+j] = GTO_nai(q, j, gto, x, y, z);

		// build contracted potential vector
		for(i=0; i < nBasis; i++)
		for(j=0; j < nBasis; j++)
		for(q=0; q < nBasis; q++){
			VqA[q] += PA[i*nBasis+j]*VAL[i]*Vqj[q*nBasis+j];
			VqB[q] += PB[i*nBasis+j]*VAL[i]*Vqj[q*nBasis+j];
		}

		// build hartree-fock exchange energy density
		value = 0.0;
		for(p=0; p < nBasis; p++)
		for(q=0; q < nBasis; q++)
			value += PA[p*nBasis+q]*VAL[p]*VqA[q] + PB[p*nBasis+q]*VAL[p]*VqB[q];
		...
		// exchange energy is multiplied by a factor of 1/2
		value = value/2.0;
	break;

	default:
		printf("uhf_getValue - type %d not supported\n", type);
		exit(-1);
	}
