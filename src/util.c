...
//
// Modified:
//			01/09/2024 - Teepanis Chachiyo - Load arguments list according to documentations 
//                                           instead of using previous hacks.
//
...
int findf(FILE *fd, int n, ...){
  ...
  va_start (argptr, n);
  for(s=0; s < n; s++){
	keys[s] = va_arg( argptr, char * );
  }
  va_end (argptr);

  for(s = 0; (ret=fscanf(fd, "%s", buf)) == 1; ){

    if( strncmp(buf, keys[s], 1024) == 0 ||
        strncmp("*", keys[s], 1024 ) == 0) s++; else s = 0;
